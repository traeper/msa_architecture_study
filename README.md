...
## 보충 의견
* JPA Entity를 Persistence 레이어에서 DTO로 변환시켜 리턴하고 있는데 이는 트랜잭션 관리, 코드 양의 증가 등 현실에서는 다소 오버스펙의 구현이 될 수 있음을 참고해주시기 바란다. 반면 Server To Server 통신을 하는 경우에는 적용할만한 예시일듯 하다.  